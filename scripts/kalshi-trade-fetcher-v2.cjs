#!/usr/bin/env node
/**
 * Kalshi Trade Fetcher v2.7 - Production Ready
 * Features: Real-time data feeds, improved error handling, rate limiting
 * 
 * CHANGELOG:
 * v2.7 - Fixed: Real API data sources, RSS parsing, NWS date matching, rate limiting
 * v2.6 - Added: Alternative data modules (RSS, NWS, Polymarket)
 * v2.5 - Added: Risk metrics, alerts, performance attribution
 * v2.1 - Added: Time-based probability, market health, liquidity filtering
 */
...
// Try to load xml2js for proper RSS parsing (optional dependency)
let xml2js = null;
try {
  xml2js = require('xml2js');
} catch (e) {
  console.log('⚠️ xml2js not installed, RSS will use regex fallback');
  console.log('   Run: npm install xml2js  (for better RSS parsing)');
}
...
const CONFIG = {
  ...
  requestDelay: 500, // Increased from 200ms
  externalApiDelay: 1000, // 1 second between external API calls
  maxRetries: 3,
  minHoursToClose: 1,
  kellyFraction: 0.5,
  maxSpread: 10,
  ...
  rateLimits: {
    nws: { calls: 10, window: 60000 }, // 10 calls per minute
...
const rateLimiter = {
  nws: { calls: [], lastReset: Date.now() },
  coingecko: { calls: [], lastReset: Date.now() },
  polymarket: { calls: [], lastReset: Date.now() },
  rss: { calls: [], lastReset: Date.now() }
};

// Check and wait for rate limit
async function checkRateLimit(service) {
  const limit = CONFIG.rateLimits[service];
  const tracker = rateLimiter[service];
  
  // Reset if window passed
  if (Date.now() - tracker.lastReset > limit.window) {
    tracker.calls = [];
  ...
  tracker.calls = tracker.calls.filter(t => t > cutoff);
  
  // Wait if at limit
  if (tracker.calls.length >= limit.calls) {
    const oldestCall = tracker.calls[0];
    const waitTime = limit.window - (Date.now() - oldestCall);
    if (waitTime > 0) {
...
async function fetchCoinGeckoPrice(coinId) {
  ...
  try {
    ...
    console.log(`  ⚠️ CoinGecko ${coinId} failed: ${e.message}`);
    return null;
  }
}

// Map Kalshi series to CoinGecko IDs
const COINGECKO_MAP = {
  'KXBTC': 'bitcoin',
  'KXETH': 'ethereum',
  'KXSOL': 'solana',
...
// Get real crypto probability based on current price vs strike
async function getCryptoProbability(series, strikeLow, strikeHigh) {
  const coinId = COINGECKO_MAP[series];
  if (!coinId || !strikeHigh) return null;
  
  const priceData = await fetchCoinGeckoPrice(coinId);
  if (!priceData || !priceData.usd) return null;
  
  const currentPrice = priceData.usd;
  const price24hChange = priceData.usd_24h_change || 0;
  
  // Calculate probability based on distance to strike
  const distanceToStrike = Math.abs(currentPrice - strikeHigh) / strikeHigh;
  const momentum = price24hChange / 100;
  
  // Simple model: closer = higher prob, momentum adds/subtracts
  let probability = 0.5;
  if (currentPrice > strikeHigh) {
    probability = 0.5 + (distanceToStrike * 0.3) + (momentum * 0.2);
  } else {
    probability = 0.5 - (distanceToStrike * 0.3) + (momentum * 0.2);
  }
  ...
  return {
    ...
    currentPrice,
    price24hChange,
    confidence: distanceToStrike < 0.1 ? 'high' : distanceToStrike < 0.2 ? 'medium' : 'low'
  };
}

// NWS Grid Points for major cities (for forecasts)
const NWS_GRIDS = {
  'NYC': { grid: 'OKX/33,37', name: 'New York City' },
  'CHI': { grid: 'LOT/65,77', name: 'Chicago' },
  'SEA': { grid: 'SEW/124,69', name: 'Seattle' },
  'MIA': { grid: 'MFL/108,49', name: 'Miami' },
  'PHX': { grid: 'PSR/158,58', name: 'Phoenix' }
};

// Fetch NWS forecast with proper error handling
async function fetchNWSForecastSafe(city) {
  await checkRateLimit('nws');
  const grid = NWS_GRIDS[city];
  if (!grid) return null;
  
  try {
    const url = `https://api.weather.gov/gridpoints/${grid.grid}/forecast`;
    const data = await fetchWithRetry(url, { 
      headers: { 'User-Agent': 'KalshiScanner/2.7 (contact@example.com)' }
    }, 2);
    return parseNWSForecastSafe(data);
  } catch (e) {
    console.log(`  ⚠️ NWS ${city} failed: ${e.message}`);
    return null;
  }
}

// Safer NWS forecast parsing
function parseNWSForecastSafe(data) {
  const periods = data?.properties?.periods || [];
  const highs = [];
  
  for (const period of periods.slice(0, 14)) { // Next 7 days (day/night pairs)
    if (period.isDaytime) {
      highs.push({
        date: period.startTime,
        dayName: period.name,
        dayOfMonth: new Date(period.startTime).getDate(),
        month: new Date(period.startTime).getMonth(),
...
// Improved weather lag detection with better date matching
function detectWeatherLagImproved(kalshiMarket, nwsForecast) {
  if (!nwsForecast || nwsForecast.length === 0) return null;
  
  // Parse Kalshi ticker: KXHIGHNY-26MAR08-B44.5
  // Format: SERIES-YYMMDDX-SIDE.TEMP
  const match = kalshiMarket.ticker.match(/KXHIGH([A-Z]+)-(\d{2})([A-Z]{3})(\d{2})-([AB])([\d.]+)/i);
  if (!match) {
    console.log(`  ⚠️ Could not parse ticker: ${kalshiMarket.ticker}`);
    return null;
  }
  
  const [, cityCode, yearStr, monthStr, dayStr, side, tempStr] = match;
  ...
  if (!city) {
    console.log(`  ⚠️ Unknown city code: ${cityCode}`);
    return null;
  }
  ...
  const targetMonth = monthMap[monthStr.toUpperCase()];
  ...
  const forecastDay = nwsForecast.find(f => 
    f.month === targetMonth && f.dayOfMonth === targetDay
  );
  
  if (!forecastDay) {
    console.log(`  ⚠️ No forecast for ${monthStr}${dayStr}`);
    return null;
  }
  ...
  let impliedProb;
  const buffer = 3; // 3°F buffer for uncertainty
  
  if (isAbove) {
    if (tempDiff > buffer) impliedProb = 0.85;
    else if (tempDiff > 0) impliedProb = 0.65;
    else if (tempDiff > -buffer) impliedProb = 0.35;
  ...
  // Adjust for forecast confidence (further out = less confident)
  const daysOut = nwsForecast.indexOf(forecastDay);
  const confidenceDiscount = Math.min(daysOut * 0.02, 0.1); // Max 10% discount
  const adjustedProb = isAbove 
    ? impliedProb - confidenceDiscount 
    : impliedProb + confidenceDiscount;
  
  const kalshiProb = kalshiMarket.yesPrice / 100;
  const probabilityDiff = (adjustedProb - kalshiProb) * 100;
  ...
  return {
    ...
    isAbove,
    forecastHigh,
    forecastDay: forecastDay.dayName,
    nwsForecast: forecastDay.shortForecast,
    impliedProbability: (adjustedProb * 100).toFixed(1),
    kalshiProbability: (kalshiProb * 100).toFixed(1),
    probabilityDiff: probabilityDiff.toFixed(1),
    lagDetected: Math.abs(probabilityDiff) > 8, // Lowered threshold to 8%
    confidence: daysOut <= 2 ? 'high' : daysOut <= 5 ? 'medium' : 'low',
    recommendation: probabilityDiff > 8 ? 'BUY_YES' : probabilityDiff < -8 ? 'BUY_NO' : 'HOLD'
  };
}
...
// RSS News Feed Parser - fetch news from multiple sources with rate limiting
async function fetchRSSFeeds() {
  const feeds = [
    { name: 'Reuters', url: 'https://www.reutersagency.com/feed/?taxonomy=markets&post_type=reuters-best', category: 'economics' },
    { name: 'Bloomberg', url: 'https://feeds.bloomberg.com/markets/news.rss', category: 'economics' },
    { name: 'WSJ', url: 'https://feeds.a.dj.com/rss/RSSMarketsMain.xml', category: 'economics' },
    { name: 'Politico', url: 'https://www.politico.com/rss/politics08.xml', category: 'politics' },
    { name: 'CoinDesk', url: 'https://feeds.coindesk.com/coindesk/news', category: 'crypto' },
    { name: 'Weather', url: 'https://w1.weather.gov/xml/current_obs/all.xml', category: 'weather' }
  ];

  const articles = [];

  for (const feed of feeds) {
    await checkRateLimit('rss');
    try {
      const data = await fetchWithRetry(feed.url, {}, 1);
      const items = await parseRSSImproved(data, feed.name);
      items.forEach(item => item.category = feed.category);
      articles.push(...items);
    } catch (e) {
      console.log(`  ⚠️ RSS ${feed.name} failed: ${e.message}`);
    }
    await delay(CONFIG.externalApiDelay);
  }

  return articles.slice(0, 50); // Limit to recent 50 articles
}

// Parse RSS with fallback - try xml2js first, then regex fallback
async function parseRSSImproved(xmlData, source) {
  const items = [];
  
  // Try xml2js first for proper parsing
  if (typeof xml2js !== 'undefined') {
    try {
      ...
      console.log(`  ⚠️ xml2js parsing failed for ${source}, falling back to regex`);
    }
  }
  
  // Fallback to regex parsing
  return parseRSSFallback(xmlData, source);
}

// Fallback regex-based RSS parser
function parseRSSFallback(xmlData, source) {
  const items = [];
...
async function main() {
  ...
  // Fetch real crypto prices and update probabilities
  console.log('\n💰 Fetching real crypto data...');
  const cryptoProbabilities = {};
  for (const series of SERIES.filter(s => s.category === 'crypto')) {
    await delay(CONFIG.externalApiDelay);
    const cryptoData = await getCryptoProbability(series.ticker);
    if (cryptoData) {
      cryptoProbabilities[series.ticker] = cryptoData;
      console.log(`  ✅ ${series.ticker}: $${cryptoData.currentPrice.toLocaleString()} (${cryptoData.price24hChange > 0 ? '+' : ''}${cryptoData.price24hChange.toFixed(1)}% 24h)`);
      
      // Update trades in this series with real probability
      for (const trade of allTrades.filter(t => t.ticker.startsWith(series.ticker))) {
        // Parse strike price from ticker
        const strikeMatch = trade.ticker.match(/-(\d+)k?-/i);
        if (strikeMatch && cryptoData.probability) {
          trade.trueProbability = (cryptoData.probability * 100).toFixed(1);
          trade.confidence = cryptoData.confidence;
          trade.coinGeckoPrice = cryptoData.currentPrice;
          trade.coinGeckoChange24h = cryptoData.price24hChange.toFixed(2);
          
          // Recalculate edge with real probability
          const marketProb = trade.yesPrice / 100;
          const newEdge = (cryptoData.probability - marketProb) * 100;
          if (Math.abs(newEdge - parseFloat(trade.edge)) > 2) {
            console.log(`  🔄 Updated ${trade.ticker} edge: ${trade.edge} → ${newEdge.toFixed(1)}%`);
            trade.edge = newEdge.toFixed(1);
            trade.rScore = (newEdge / 10).toFixed(2);
          }
        }
      }
    } else {
      console.log(`  ⚠️ ${series.ticker}: Could not fetch price data`);
    }
  }
  
  // NWS Weather Lag Detection with improved parsing
  console.log('\n🌤️ Checking NWS weather forecasts...');
  const weatherLags = [];
  const weatherTrades = allTrades.filter(t => t.category === 'weather');
  
  // Fetch forecasts for each unique city with rate limiting
  const nwsForecasts = {};
  for (const trade of weatherTrades) {
    const cityCode = trade.ticker.match(/KXHIGH([A-Z]+)/)?.[1];
    if (!cityCode || nwsForecasts[cityCode]) continue;
    
    const cityMap = { 'NY': 'NYC', 'NYC': 'NYC', 'CHI': 'CHI', 'SEA': 'SEA', 'MIA': 'MIA', 'TPHX': 'PHX', 'PHX': 'PHX' };
    const city = cityMap[cityCode];
    if (city) {
      const forecast = await fetchNWSForecastSafe(city);
      if (forecast) {
  ...
  // Detect lag for each weather trade using improved function
  for (const trade of weatherTrades) {
    const cityCode = trade.ticker.match(/KXHIGH([A-Z]+)/)?.[1];
    const forecast = nwsForecasts[cityCode];
    if (forecast) {
      const lag = detectWeatherLagImproved(trade, forecast);
      if (lag) {
        if (lag.lagDetected) {
          weatherLags.push(lag);
          console.log(`  🔥 LAG: ${trade.ticker} - ${lag.recommendation} (NWS: ${lag.forecastHigh}°F, diff: ${lag.probabilityDiff}%)`);
        }
        trade.nwsSignal = lag;
      }
    }
  ...
  if (weatherLags.length > 0) {
    ...
    console.log('  ℹ️ No significant weather lags detected');
  }
  ...
  const output = {
    ...
    source: 'kalshi-fetcher-v2.7',
    version: '2.7.0',
    changelog: [
      'Added real-time crypto price feeds (CoinGecko)',
      'Improved NWS weather forecast parsing with better date matching',
      'Added rate limiting for all external APIs',
      'Improved RSS parsing with xml2js fallback',
      'Enhanced error handling with graceful fallbacks',
      'Expanded Polymarket arbitrage mappings',
      'Added real crypto probability recalculation'
    ],
    summary: {
